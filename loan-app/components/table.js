...
export const Pagination = ({pageNumber, listLength, onClick}) => {
    ...
    let pageNumbers = []
      if (noOfPages <10) {
        ...
        if (pageNumber === 1) {
          pageNumbers.push(<span style={{backgroundColor:"grey"}} onClick={() => onClick(1)}>1</span>)
        } else {
        ...
        if (pageNumber === noOfPages) {
          pageNumbers.push(<span onClick={() => onClick(pageNumber-2)}>{pageNumber-2}</span>)
        }
        // Print previous number button if pageNumber > 3
        if (pageNumber > 2) {
          pageNumbers.push(<span onClick={() => onClick(pageNumber-1)}>{pageNumber-1}</span>)
        }
        //Print current page number button as long as it not the first or last page
        if (pageNumber !== 1 && pageNumber !== noOfPages) {
            pageNumbers.push(<span style={{backgroundColor:"grey"}} onClick={() => onClick(pageNumber)}>{pageNumber}</span>)
        }
        ...
        if (noOfPages > 1) {
          if (pageNumber === noOfPages) {
            pageNumbers.push(<span style={{backgroundColor:"grey"}} onClick={() => onClick(noOfPages)}>{noOfPages}</span>)
          } else {
            pageNumbers.push(<span onClick={() => onClick(noOfPages)}>{noOfPages}</span>)
          }
          
        }
      }
    return (
      <div style={{flexDirection: "row", display:'flex', width:"100%", justifyContent:"center", backgroundColor:"white"}}>
        {pageNumbers}
      </div>
    )
}
